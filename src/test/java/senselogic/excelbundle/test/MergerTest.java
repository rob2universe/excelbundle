...
{
	private File root = new File("target/test-classes/");
	...
	public void testMerge() throws IOException
	{
		try
		{
			...
			// TODO WHy do those fail? Are statistics calculated correctly?
...
//						result.getLanguageFile(langFile.getPath())));
		...
		{
			new File("target/test-classes/foobar1_sv.properties").delete();
			new File("target/test-classes/foobar2_sv.properties").delete();
			new File("target/test-classes/foobar1_en.properties").delete();
			new File("target/test-classes/foobar2_en.properties").delete();
		}
	}
}